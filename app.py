...
with mysql.connector.connect(host=host,user=user,password=password,db=db,port=port) as conn:
    ...
    cursor.execute("create table if not exists reviews(username varchar(30),itemid varchar(30),title tinytext,review text,rating int,primary key(username,itemid) foreign key(username) references signup(username) on update cascade on delete cascade,foreign key(itemid) references additems(itemid) on update cascade on delete cascade,date datetime default now())")   
